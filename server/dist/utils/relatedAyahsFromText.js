...
const AYAH_REF_REGEX = /(?<!\[)(Sure|Surah|Sura)\s*(\d{1,3})(\s*\([^)]{1,80}\))?\s*[,:\-–]?\s*(Vers|Aya|Ayah)\s*(\d{1,3})/gi;
const PAREN_SURAH_AYAH_REGEX = /(?<!\[)(Sure|Surah|Sura)\s+([^\n(]+?)\s*\((\d{1,3})\s*:\s*(\d{1,3})\)/gi;
const SHORT_COLON_REGEX = /(?<!\[)\b(Sure|Surah|Sura)\s*(\d{1,3})\s*:\s*(\d{1,3})\b/gi;
const NAMED_SURAH_VERS_REGEX = /(?<!\[)(Sure|Surah|Sura)\s+([^,\n]+?)\s*,\s*(?:Vers|Aya|Ayah)\s*(\d{1,3})/gi;
function addRef(refs, surahId, ayahNumber, label) {
    ...
    refs.set(k, {
        surahId,
        ayahNumber,
        shortLabel: trimmed && trimmed.length > 0 ? trimmed.slice(0, 120) : undefined,
    });
}
/**
 ...
 */
export function extractRelatedAyahsFromText(text) {
    const refs = new Map();
    if (!text || !text.trim())
        return [];
