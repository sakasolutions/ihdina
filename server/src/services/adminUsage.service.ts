...
export async function getUsageDailyAggregates(days: number): Promise<DailyUsageRow[]> {
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      avgLatencyMs:
        r.avgLatencyMs == null || Number.isNaN(Number(r.avgLatencyMs))
          ? null
