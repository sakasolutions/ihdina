...
function validateFollowUpInput(input) {
    ...
    for (const m of input.history) {
        if (!m || typeof m.role !== "string" || typeof m.content !== "string") {
            throw new AppError(ErrorCodes.INVALID_INPUT, "Each history item must have string role and content.", 400);
        }
        if (!allowedRoles.has(m.role)) {
            throw new AppError(ErrorCodes.INVALID_INPUT, `Invalid message role: ${m.role}`, 400);
        }
...
function questionWithLanguageHint(language, question) {
    const q = question.trim();
    const lang = language?.trim().toLowerCase() ?? "";
    if (!lang || lang === "de" || lang === "de-de" || lang === "german")
        return q;
...
export async function followUpVerse(input) {
    ...
    if (!user.isPro) {
        ...
        throw new AppError(ErrorCodes.PRO_REQUIRED, "Pro subscription is required for follow-up questions.", 403);
    }
    ...
    if (current >= MAX_FOLLOWUPS_PER_VERSE) {
        ...
        throw new AppError(ErrorCodes.FOLLOWUP_LIMIT_REACHED, "Maximum follow-ups for this verse have been used.", 403);
    }
    const messages = [
        ...input.history.map((h) => ({
            role: h.role,
