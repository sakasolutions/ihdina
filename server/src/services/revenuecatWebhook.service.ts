...
export async function processRevenueCatEvent(
  ...
  log.info(
    { userId: user.id, installId: user.installId, isPro, expiresAt },
    `RevenueCat webhook: user updated → isPro=${isPro}`
  );
}
...
async function findUser(
  ...
  const candidates = [appUserId, originalAppUserId].filter(Boolean) as string[];
...
function parseExpiresAt(raw: unknown): Date | null {
  if (typeof raw === "number") return new Date(raw); // ms timestamp
