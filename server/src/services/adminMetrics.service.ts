...
export type AiDayPoint = { day: string; total: number; errors: number };
export type EndpointCount = { endpoint: string; count: number };
export type ErrorCodeCount = { errorCode: string; count: number };
export type FeedbackScreenRow = { screen: string; count: number; avgRating: number | null };
...
function buildInsights(p: {
  ...
  if (p.ai7d === 0) {
    ...
    out.push(
      `KI-Fehlerquote 7 Tage bei ca. ${Math.round(p.errorRate * 100)} % — Endpoint- und Fehlercode-Tabelle prüfen, Release/API-Keys testen.`
    );
  ...
  if (p.totalUsers > 0 && p.new7 > 0) {
    ...
    if (actRatio < 0.15 && p.totalUsers > 50) {
      out.push(
        "Relativ wenige Nutzer mit API-Aktivität in 7 Tagen vs. Bestand — Retention/Öffnen-Frequenz oder Messpunkte prüfen (lastSeenAt bei API-Nutzung)."
      );
...
export async function getAdminMetricsOverview(): Promise<AdminMetricsOverview> {
  ...
  const byDayRows = await prisma.$queryRaw<{ d: Date; total: unknown; errors: unknown }[]>(Prisma.sql`
    SELECT
  ...
  const byDay: AiDayPoint[] = byDayRows.map((r) => ({
    day: r.d instanceof Date ? r.d.toISOString().slice(0, 10) : String(r.d).slice(0, 10),
    total: toInt(r.total),
    errors: toInt(r.errors),
  }));

  const byEpRows = await prisma.$queryRaw<{ endpoint: string; cnt: unknown }[]>(Prisma.sql`
    ...
    LIMIT 12
  `);
  const byEndpoint7d: EndpointCount[] = byEpRows.map((r) => ({
    endpoint: r.endpoint,
    count: toInt(r.cnt),
  }));

  const errCodeRows = await prisma.$queryRaw<{ ec: string; cnt: unknown }[]>(Prisma.sql`
    ...
    LIMIT 10
  `);
  const topErrorCodes7d: ErrorCodeCount[] = errCodeRows.map((r) => ({
    errorCode: r.ec,
    count: toInt(r.cnt),
  }));

  const screenRows = await prisma.$queryRaw<{ scr: string; cnt: unknown; av: unknown }[]>(Prisma.sql`
    SELECT
    ...
    LIMIT 14
  `);
  const byScreen7d: FeedbackScreenRow[] = screenRows.map((r) => ({
    screen: r.scr,
    count: toInt(r.cnt),
